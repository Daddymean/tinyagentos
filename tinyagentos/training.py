...
class TrainingManager(BaseStore):
    ...
    async def _post_init(self):
        ...
        presets = [
            (
                "quick",
                "Quick",
                "1 epoch, small LoRA rank -- fast results, lower quality",
                {
                    "epochs": 1,
                    "lora_rank": 8,
                    "lora_alpha": 16,
            ...
            (
                "balanced",
                "Balanced",
                "3 epochs, medium rank -- good balance",
                {
                    "epochs": 3,
                    "lora_rank": 16,
                    "lora_alpha": 32,
            ...
            (
                "thorough",
                "Thorough",
                "5 epochs, larger rank -- best quality, slower",
                {
                    "epochs": 5,
                    "lora_rank": 32,
                    "lora_alpha": 64,
    ...
    async def delete_job(self, job_id: str) -> bool:
        cursor = await self._db.execute(
            "DELETE FROM training_jobs WHERE id = ?", (job_id,)
        )
        await self._db.commit()
    ...
    async def get_presets(self) -> list[dict]:
        async with self._db.execute(
            "SELECT id, name, description, config FROM training_presets"
        ) as cursor:
            return [
                {
                    "id": r[0],
